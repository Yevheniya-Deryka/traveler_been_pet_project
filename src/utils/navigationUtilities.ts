...
import type { MainStackParamList, NavigationProps } from "@/navigators/MainStackNavigator/MainStackNavigator.types"
...
export function getActiveRouteName(state: NavigationState | PartialState<NavigationState>): string {
	...
	if (!route.state) return route.name as keyof MainStackParamList

	// Recursive call to deal with nested routers
	return getActiveRouteName(route.state as NavigationState<MainStackParamList>)
}
...
export function useNavigationPersistence(storage: Storage, persistenceKey: string) {
	...
	const routeNameRef = useRef<keyof MainStackParamList | undefined>(undefined)
	...
	const onNavigationStateChange = (state: NavigationState | undefined) => {
		...
		if (state !== undefined) {
			...
			routeNameRef.current = currentRouteName as keyof MainStackParamList
 ...
 * @param {Parameters<typeof navigatioAppNavigatornRef.resetRoot>[0]} state - The state to reset the root to.
 * @returns {void}
 */
